...
public abstract class Ship {
	...
	/**********CONSTRUCTORS***********/
	/**
	 * Default constructor that creates a basic working ship.
	 */
	protected Ship() {
		this.name = "Unnamed Ship";
		this.type = ShipType.WORKING;
		this.hull = 100;
		this.status = MissionStatus.AVAILABLE;
	}

	/**
	 * Full constructor used to create a ship with a chosen name, role, and hull value.
	 * Mission state starts as AVAILABLE with no active mission timer.
	 *
	 * @param name display name for the ship
	 * @param type ship category (fighter or working)
	 * @param hull hull integrity value
	 */
	protected Ship(String name, ShipType type, int hull) {
		setName(name);
		setType(type);
		setHull(hull);
		this.status = MissionStatus.AVAILABLE;
	...
	/**
	 * Sets ship type defaulting to WORKING when null.
	 *
	 * @param type ship role enum
	 */
	public void setType(ShipType type) {
		this.type = (type == null) ? ShipType.WORKING : type;
	}
	...
	@Override
	public int hashCode() {
		return Objects.hash(name, type, hull, status);
	}

	@Override
	public String toString() {
		return String.format(
				"Ship[name=%s, type=%s, hull=%d, status=%s]",
				name, type, hull, status
		);
	}

	/********* HELPER METHODS *********/
	/********* ABSTRACT TYPE MARKER *********/
}
